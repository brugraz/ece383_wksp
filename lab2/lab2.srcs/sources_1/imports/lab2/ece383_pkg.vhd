...
constant RDWRADDR_WIDTH     : integer := 10;
...
type channel_t is record
  to_ac: std_logic_vector(AC_BUS_WIDTH-1 downto 0);
  from_ac: std_logic_vector(AC_BUS_WIDTH-1 downto 0);
  incoming_sample: std_logic_vector(READWRITE_WIDTH-1 downto 0);
  current_sample: std_logic_vector(READWRITE_WIDTH-1 downto 0);
  prev_sample: std_logic_vector(READWRITE_WIDTH-1 downto 0);
  to_bram: std_logic_vector(READWRITE_WIDTH-1 downto 0);
  from_bram: std_logic_vector(READWRITE_WIDTH-1 downto 0);
  active: std_logic;
  en: std_logic;
end record;
...
type external_config_t is record
  wr_addr: std_logic_vector(RDWRADDR_WIDTH-1 downto 0);
  write_en: std_logic;
  sel: std_logic;
  Lbus: std_logic_vector(READWRITE_WIDTH-1 downto 0);
  Rbus: std_logic_vector(READWRITE_WIDTH-1 downto 0);
end record;
...
constant SIM_LIVE_SWITCH : integer := 2;
...
constant bit_CW_WREN        : integer := 2;
constant bit_SW_TRIG        : integer := 2;
constant bit_SW_LASTADDR    : integer := 1;
constant bit_SW_READY       : integer := 0;
...
component vga_signal_generator is port(
  clk      : in STD_LOGIC;
  reset_n  : in STD_LOGIC;
...
component color_mapper is port(
  color    : out color_t;
...
component trigger_detector is Port (
  clk              : in  std_logic;
  reset_n          : in  std_logic;
