...
begin
	...
	-- Add code here
  ch1.active <= '1' when position.row = apply_offset(ch1.from_bram(15 downto 6));
...
Audio_Codec : Audio_Codec_Wrapper Port map (
  ...
  sim_live => is_live);  --  '0' simulate audio; '1' live audio
    ...
    port map (
      ...
      WRADDR => exLBus,                -- Input write address, width defined by write port depth
...
rightChannelMemory : BRAM_SDP_MACRO
  ...
  port map (
    ...
    WE => "11",                        -- Input write enable, width defined by write port depth
    WRADDR => exRBus,                -- Input write address, width defined by write port depth
    WRCLK => clk,                    -- 1-bit input write clock
    WREN => cw_write_en);                -- 1-bit input write port enable
  -- End of BRAM_SDP_MACRO_inst instantiation
